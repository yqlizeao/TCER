...
基于真实 Claude Code 会话数据，度量「每消耗多少 Token、产出多少有效代码」的多维效率体系。指标框架为原创的五层体系，`tcer/` 是其完整可运行的 Python 实现。
...
```
.
├── CLAUDE.md                                    # 项目设计/规格文档（数据格式、公式、实现路径）
└── tcer/                                        # 实现：纯 Python 标准库，零第三方依赖
    ├── src/tcer/                                # 源码（10 模块，约 2000 行）
    ├── tests/                                   # 测试（6 文件，62 项全过）
    ├── conftest.py                              # 让 python -m pytest 找到 src/（免安装）
    └── README.md                                # 工具自身的详细说明
```

## 这个工具做什么

`tcer` 直接解析 Claude Code 写在 `~/.claude/projects/<项目哈希>/*.jsonl` 的会话文件（**离线、零 API 调用、零埋点**），汇总 Token 用量、统计代码增删，算出一套效率指标，并提供命令行与图形界面两种查看方式。
...
| `analyze.py` | 编排层：项目→会话→指标，CLI 与 GUI 共用 |
| `report.py` | 终端表格、聚合块、CTEI 条形图、CSV/JSON 导出 |
| `cli.py` | 命令行入口：`list` / `report` / `gui` |
| `gui.py` | Tkinter 图形界面（全中文 + 指标说明） |
...
- 手改 JSON 覆盖任意系数
- 用 `--compute-baselines` 从自己积累的数据计算中位数/均值，建立个人基准（框架 §8.3 建议）
- CLI 的 `--baseline-*` 参数可逐次覆盖（不改文件）
...
这些范围来自 TCER 项目自身数据与原始框架参考集的观察，**不是理论阈值**——实际"健康值"因任务类型、代码库规模、个人风格而异。用 `--compute-baselines` 建立你自己的参考线。
...
**F1 风险（Write 覆写已有文件）**：`file_lines` 在每个会话开始时为空，所以 `Write` 首次遇到某文件时「旧行数」被假设为 0——对新文件正确，对覆写已有文件错误（整个新内容算 added，删除被漏掉）。`Edit` 只看 delta，不受影响。报告的 `unseen_writes` 计数（Quality L3 层）= 本会话首次 Write 的文件数，是 F1 暴露面上界。实测 TCER 的真实工作流（新文件用 Write、改老文件用 Edit）**偏差 0%**，但换了工作流就会咬人。用 `calibrate_loc.py`（与 git 历史对照）可精确量化偏差。核心保持 git-free，风险通过计数器可见。
...
基准（默认取原始框架 16-会话参考数据集的中位数/均值，可用 `--baseline-*` 覆盖）：
...
```bash
cd tcer/src
python -m tcer.gui                                 # 启动图形界面（全中文）
python -m tcer.cli list                            # 列出本地所有项目
...
> 从 `src/` 目录运行会自动把 `tcer` 包加入导入路径，零配置——不需要 editable 安装，也不会往 PATH 里塞 console 命令。`python -m tcer.gui` 走 GUI，`python -m tcer.cli` 走命令行（注意 `python -m tcer` 直接跑不行，包内无 `__main__`）。

`--project` 接受名称或哈希（如 `TCER` 自动匹配对应项目目录）。**子代理（subagent）会并入其父会话**作为同一个 session：它们的 Token 与代码计入父会话（保留真实成本），但不单独计为一个 session——所以会话数与 cc-switch 一致。用 `--no-subagents` 可完全排除子代理数据。

### LOC 校准工具（F1 偏差量化）

`calibrate_loc.py`（仓库根目录）用 git 历史对照 tcer 的 `net_loc`，量化 F1 风险的真实偏差。tcer 核心保持 git-free，此脚本**仅用于验证**：

```bash
python calibrate_loc.py --project TCER               # 全项目校准
python calibrate_loc.py --project TCER --session <id>  # 单会话
python calibrate_loc.py --project TCER --no-subagents  # 排除 subagent
```

输出逐会话的「tcer 净增 vs git 真实净增」，以及总偏差百分比。TCER 项目实测 **0% 偏差**（28 个 `unseen_writes` 都落在新文件上，工作流是新文件 Write、老文件 Edit）。报告里的 `unseen_writes` 计数（Quality L3 层）给出 F1 暴露面上界，此脚本给出精确偏差。

### 图形界面

`python -m tcer.gui`（从 `tcer/src` 目录运行）打开桌面窗口：左侧项目列表，右侧指标卡片 + 逐会话表格（含**时间列**、按 CTEI 评级着色）+ **图表区**。图表可切换：**TCER / CTEI / CPE / 缓存命中 / 成本的时间趋势折线**（横轴为会话开始时间，TCER 趋势叠加框架基准线），或 CTEI 评级排名条形图。顶部可切换任务类型、排除子代理，改动即时重算。**鼠标悬停卡片**或点**「指标说明」**可看每个指标的中文解释。纯标准库 `tkinter`，分析在后台线程运行不卡界面。
...
- **软指标可配置**：`data/composite_baselines.json` 存储 TTAF / CTEI 基准 / PSAC / CHR 权重，可手改或用 `--compute-baselines` 从积累数据建立个人基准。
- **测试 62 项全过**（`reader` / `paths` / `metrics` / `report` / `loc` / `pricing`）。
- **硬数据可靠性已审计**：F1（Write 覆写已有文件）用 `unseen_writes` 计数暴露风险、`calibrate_loc.py` 量化偏差（实测 0%）；F4（行数口径）已统一 `session_loc` 与 `tree_loc` 为 universal newlines；时间戳盲区（仅零 usage 回合的会话）已修复。
- 纯 Python 标准库，零第三方运行时依赖；版本 0.3.0。
