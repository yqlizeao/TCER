...
基于真实 Claude Code 会话数据，度量「每消耗多少 Token、产出多少有效代码」的多维效率体系。理论框架来自本仓库的研究报告，`tcer/` 是其完整可运行的 Python 实现。
...
实现已验证能**复现研究报告发布的逐会话评分（误差 <0.1%）**。成本按**各模型 list 价**分别估算并求和（价表见 `tcer/src/tcer/data/model_pricing.json`，≈160 模型；未知模型回退 Anthropic 标价 input \$3 / output \$15 / cache-write \$3.75 / cache-read \$0.30 每百万 Token）。
...
按时间顺序回放会话里的文件改写工具调用，只统计代码后缀文件（`.py/.js/.ts/.md/...`）。`nlines(s)` = 字符串行数。维护「本会话内各文件当前行数」`file_lines`：
...
代码库累计行数 `loc_accumulated` = `tree_loc(工作目录)`：扫描目录、跳过 `EXCLUDE_DIRS`、累加代码后缀文件的行数。
...
TTAF 取值（报告 §6.4）：新功能 1.00 · 功能扩展 0.85 · 调试 0.40 · 重构 0.50 · 审查 0.20 · 测试 0.90。
...
基准（默认取研究报告 16-会话数据集的中位数/均值，可用 `--baseline-*` 覆盖）：
`TCER_baseline = 76.59`、`NCPI_baseline = 0.101`、`CPE_baseline = 8.22`。

评级：**优秀 >2 · 良好 1~2 · 中等 0.5~1 · 低效 0.1~0.5 · 极端低效 <0.1**。该实现已用测试断言能**复现报告发布的逐会话 CTEI（误差 <0.1%）**。
...
`python -m tcer.gui`（从 `tcer/src` 目录运行）打开桌面窗口：左侧项目列表，右侧指标卡片 + 逐会话表格（含**时间列**、按 CTEI 评级着色）+ **图表区**。图表可切换：**TCER / CTEI / CPE / 缓存命中 / 成本的时间趋势折线**（横轴为会话开始时间，TCER 趋势叠加报告基准线），或 CTEI 评级排名条形图。顶部可切换任务类型、排除子代理，改动即时重算。**鼠标悬停卡片**或点**「指标说明」**可看每个指标的中文解释。纯标准库 `tkinter`，分析在后台线程运行不卡界面。
...
- 报告的五层指标体系（L1 原始 / L2 效率 / L3 质量 / L4 经济 / L5 综合）已全部实现；L3 目前覆盖 churn 率（圈复杂度、覆盖率需引入 radon/lizard 等外部工具，留作 opt-in）。
- 成本逐模型计价：价表 `data/model_pricing.json`（≈160 模型，源自 cc-switch），未知模型回退 default。
- **测试 62 项全过**（`reader` / `paths` / `metrics` / `report` / `loc` / `pricing`）。
- 纯 Python 标准库，零第三方运行时依赖；版本 0.2.0。

> 注：TTAF 取值以研究报告 §6.4 为准（重构 0.50、审查 0.20），代码、CLAUDE.md 与本文档三者一致。
