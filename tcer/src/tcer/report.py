...
def to_markdown(reports: list[SessionReport], agg: SessionReport, n_sessions: int,
    ...
    """Export a lightweight Markdown report (shareable summary + key metrics).

    Designed for embedding in PRs, docs, or wiki pages. Includes aggregate stats,
    per-session table (top metrics only), and an ASCII CTEI bar chart.
    """
    u = agg.usage
    lines = [
        ...
        f"**{fmt_money(agg.cost)}** @ list price",
        "",
        "## Summary",
        "",
        f"| Metric | Value | Note |",
        f"|--------|-------|------|",
        f"| **Net LOC** | {fmt_int(agg.net_loc)} | Tool-call derived (git-free) |",
        f"| **TCER** | {fmt_float(agg.tcer, '0.00')} LOC/Mt | Token → Code efficiency |",
        f"| **CPE** | {fmt_money(agg.cpe)}/kLOC | Cost per 1000 lines |",
        f"| **CHR** | {fmt_pct(agg.chr)} | Cache hit ratio (lower cost) |",
        f"| **Churn** | {fmt_pct(agg.churn_ratio)} | Rework fraction (deleted/added) |",
        f"| **CTEI** | {fmt_float(agg.ctei, '0.000')} | Composite efficiency index |",
        f"| **Grade** | {agg.grade or '-'} | CTEI rating |",
        "",
    ]
    if agg.unseen_writes:
        lines.append(f"⚠️ **{agg.unseen_writes} unseen Writes** (F1 exposure: first Write to files may assume prior size = 0)")
        lines.append("")

    # Per-session table (compact)
    lines += [
        "## Sessions",
        "",
    ...
    for r in reports:
        sid = (r.meta.session_id or r.meta.path.stem)[:12]
        lines.append(
            f"| `{sid}` | {fmt_int(r.usage.total)} | {fmt_pct(r.chr)} | "
            f"{fmt_int(r.net_loc)} | {fmt_float(r.tcer, '0.0')} | "
            f"{fmt_float(r.ctei, '0.00')} | {r.grade or '-'} |"
        )

    # ASCII CTEI chart (reuse existing renderer, strip ANSI)
    chart_ascii = ctei_chart(reports, color=False)
    if chart_ascii:
    ...
    lines += [
        "",
        f"---",
        f"*Generated by [TCER](https://github.com/your-org/TCER) v{_version()} · "
        f"Models: {models_label(u)} · Window: {fmt_ms(u.started_at)} → {fmt_ms(u.ended_at)}*",
    ]
    return "\n".join(lines)


def _version() -> str:
    """Inline version read (avoid circular import from __init__)."""
    try:
