...
25. **排名页**：综合效率分默认即可计算，排名页（`views.ScoreRankingView`）默认就有数据；仅当会话无 score（no_loc 或无净增行/成本→无产出轴）时回退按 TCER 排名并显示提示条。得分构成面板展示三正交轴（半饱和值 0–1、中点 0.5=与基准持平）+ 加权合成，标签取自 `metric_defs.SCORE_AXES`（SSOT）。`launch.bat` 优先 `pyw`/`pythonw`（`start` 分离，无残留控制台；文件为 GBK 编码保 cmd 中文注释）。
