...
基于真实 AI 编程助手会话数据，构建多维 AI 编程效率计量体系（TCER + 综合效率分）。支持六个数据来源：Claude Code（`~/.claude`）、Codex（`~/.codex`）、OpenCode（`~/.local/share/opencode`）、Grok / grok build CLI（`~/.grok`）、Oh My Pi / omp（`~/.omp`）、Pi / earendil-works/pi（`~/.pi`）。
