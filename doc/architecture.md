...
| `tcer/core/paths.py` | 定位 `~/.claude` / `~/.codex` / `~/.local/share/opencode`、项目哈希编解码、统一项目引用 |
...
5. **纯离线**：不依赖任何版本管理工具（git 等）、不做任何联网操作。所有数据来自本地 `~/.claude/`、`~/.codex/` 与 `~/.local/share/opencode/`。
