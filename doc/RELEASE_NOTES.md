...
- **配置移入 `tcer_ui.json` 的 `upload` 段**：服务器地址 / API Token / 是否附带明细不再走 `.env` 环境变量，集中到与界面偏好同一份 `tcer_ui.json`，随整体包分发、随包覆盖。
- **弹窗内可编辑**：上传对话框新增服务器地址、API Token、明细开关的编辑表单；点「立即上传」时先保存配置再上传，无需手改文件。
