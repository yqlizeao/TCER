...
基准默认值来自 `tcer/config/composite_baselines.json`（当前 TCER≈26.22, CPE≈13.62，源自框架参考数据集的中位数）。数值以配置为唯一真理源——「保存个人基准」会覆盖它，GUI 的「TCER 基准 / 成本基准」两格实时显示当前生效值。切勿在代码/文档中硬编码具体基准数字（历史上曾散落 76.59/8.22 导致雷达图刻度失真）。
...
| 指标 | 优秀 | 良好 | 需改进 | 说明 |
|------|------|------|--------|------|
| **TCER** | >80 | 40–80 | <40 | 经验带，与配置基准独立（基准仅用于 CTEI 打分） |
...
| **CTEI** | (80/BL_TCER)×(BL_CPE/9.11)×1.480，此处取示例基准 BL_TCER=76.59、BL_CPE=8.22 | **≈ 1.39 → 良好** |

> 算例中的基准 (76.59 / 8.22) 仅为演示常量；实际计算以 `composite_baselines.json` 的生效值为准（当前默认 26.22 / 13.62，改基准会改变 CTEI 绝对值，但不影响排名相对关系）。
