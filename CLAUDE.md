...
**仅提供 GUI**：TCER 产品以 Tkinter 桌面界面为唯一入口（`python -m tcer`）。CLI 已于 2026-06 重构时移除。**纯离线工具**：不依赖任何版本管理工具（git 等）、不做任何联网操作，所有数据来自本地 `~/.claude/` 目录的 JSONL 文件。
...
- **配置文件**：`tcer/config/model_pricing.json`（`_meta` 记录来源/抓取日期；`default` 为未知模型回退价；`models` 按 model_id 索引，字段 `input/output/cache_read/cache_write`）
- **加载与解析**：`tcer/core/pricing.py` —— `resolve(model)` 按「精确 id → 最长前缀 id（兼容 Claude Code 追加的 `[1m]`/日期后缀）→ default」解析；`label(model)` 返回友好名；`default_pricing()` 返回回退价
...
## 源码模块（GUI-only 架构，2026-06 重构后）

| 模块 | 职责 |
|------|------|
| `tcer/core/reader.py` | JSONL 发现/解析、`isMeta` 过滤、head/tail 取样、时间戳归一化、`message.usage` 聚合 |
| `tcer/core/paths.py` | 定位 `~/.claude`、项目哈希编解码 |
| `tcer/core/loc.py` | **git-free** 代码量统计：`session_loc`（从工具调用统计增删）+ `tree_loc`（扫描工作目录） |
| `tcer/core/metrics.py` | 全部公式：TCER/CHR/CPE/CAF/TTAF/TA-TCER/PSAC/NCPI/churn/CTEI + 评级 + 逐模型成本 + compute/save_baselines |
| `tcer/core/pricing.py` | 逐模型计价：从 `tcer/config/model_pricing.json`（≈160 模型）解析 `$/MTok`，未知模型回退 default |
| `tcer/core/models.py` | 数据类（`TokenUsage` / `ModelUsage` / `SessionMeta` / `SessionReport`） |
| `tcer/core/analyze.py` | 编排层：项目→会话→指标，GUI 调用 |
| `tcer/core/calibrate.py` | LOC 精度校准：对照 git 历史量化工具调用 LOC 偏差 |
| `tcer/core/format.py` | 纯值格式化器（千分位/百分比/时间戳/模型名），GUI 与导出层共用 |
| `tcer/core/export.py` | JSON/CSV/Markdown 序列化 + CTEI 排名数据（`ctei_ranking`）+ 文本条形图 |
...
├── views.py        # 面板：FilterBar/ProjectColumn/SessionColumn/MetricPanel + CteiBarChart/TrendChart
└── popups.py       # 弹窗：指标说明/会话详情/工具调用/校准/基准/高级选项
```
...
4. **库层不动**：`tcer/core/` 下的 reader/loc/metrics/pricing/models/paths/analyze/calibrate 有完整测试覆盖，改动需谨慎。GUI 改动集中在 `tcer/gui/` 包内。
...
4. **subagent 数据**：主 session 的 subagent 对话存储在 `<sessionId>/subagents/` 子目录下，文件名形如 `agent-<id>.jsonl`。**本工具的做法**：用 `reader.parent_session_id()` 把每个 subagent 按 `<sessionId>` **并入其父会话**作为同一个 session——token 与 LOC 计入父会话（保留真实成本），不单独计为一个 session。子代理始终并入主会话，不可排除。
...
10. **★ LOC 的 F1 风险（Write 覆写已有文件）**：`loc.session_loc` 对每个会话从空的 `file_lines={}` 开始回放工具调用。`Write` 首次遇到某文件时，将其原长度假设为 0——这对**新文件**是对的，对**覆写已有文件**是错的（整个新内容被计为 added，删除被漏掉）。`Edit` 只看 `new_string - old_string` 的行数差，不受影响。工具在 Quality (L3) 层报告 **`unseen_writes` 计数**——即「本会话首次 Write 的文件数」，作为 F1 暴露面上界。实测 TCER 项目 9 个会话的真实工作流（新文件用 Write、改老文件用 Edit）**偏差为 0%**（28 个 unseen Write 都落在新文件上），但受控实验证明 F1 可造成单次 100 行的偏差。**精确量化偏差**用 GUI 的「校准 LOC」功能（`calibrate.calibrate_project`，对照 git 历史取每个 Write 发生时文件的真实原长，对照 tcer 假设的 0）。这是潜伏 bug：今天不触发，换了工作流（或别人用 Write 覆写）就会污染 TCER。核心保持 git-free；`unseen_writes` 让风险可见、可追踪。
